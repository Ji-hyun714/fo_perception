...
bool SF2Publisher::init(){
    ...
    radar_obs_sub_ = this->create_subscription<fo_msgs::msg::RadarTrArray>(
            "/radar_tr", 10,
...
void SF2Publisher::callback_lidar_general(const std_msgs::msg::UInt8MultiArray::SharedPtr msg){
    lidar2_fl_fail_flag = static_cast<uint8_t>(msg->data[0]);
    lidar2_fl_alive_counter = static_cast<uint8_t>(msg->data[1]);
    lidar2_fr_fail_flag = static_cast<uint8_t>(msg->data[3]);
    lidar2_fr_alive_counter = static_cast<uint8_t>(msg->data[4]);
    lidar2_r_fail_flag = static_cast<uint8_t>(msg->data[6]);
    lidar2_r_alive_counter = static_cast<uint8_t>(msg->data[7]);
...
void SF2Publisher::callback_radar_obj(const fo_msgs::msg::RadarTrArray::SharedPtr msg){
    radar_obj_data_list_ = {};

    for (auto &radar_obj : msg->tracks) {
        if(radar_obj.track_pos_x == 0 && radar_obj.track_pos_y == 0){
            continue;
        }

        SFObjs radar_obj_data_{};

        radar_obj_data_.track_valid = 1;
        radar_obj_data_.sf2_track_status = static_cast<uint8_t>(radar_obj.track_status);
        radar_obj_data_.sf2_track_pos_y = radar_obj.track_pos_y;
        radar_obj_data_.sf2_track_pos_x = radar_obj.track_pos_x;
        radar_obj_data_.sf2_rolling_count = static_cast<uint8_t>(radar_roilling);
        radar_obj_data_.sf2_track_type = static_cast<uint8_t>(radar_obj.track_type);
        radar_obj_data_.sf2_track_vel_y = radar_obj.track_vel_y;
        radar_obj_data_.sf2_track_vel_x = radar_obj.track_vel_x;
        radar_obj_data_.sf2_add_info_status = 1;
        radar_obj_data_.sf2_track_id = static_cast<uint8_t>(radar_obj.track_id);
        radar_obj_data_.used_radar2_track_id_first = static_cast<uint8_t>(radar_obj.track_id);
        radar_obj_data_.used_radar2_track_id_second = static_cast<uint8_t>(radar_obj.track_id);

        radar_obj_data_.length = radar_obj.length > 0 ? static_cast<float>(radar_obj.length) : radar_anchor_l;
        radar_obj_data_.width = radar_obj.width > 0 ? static_cast<float>(radar_obj.width) : radar_anchor_w;
        radar_obj_data_.height = 1.0;
        radar_obj_data_.confidence = conf_base - 0.1f + static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX) * 0.1f;

        radar_obj_data_list_.push_back(radar_obj_data_);
    }
...
void SF2Publisher::fusion(){
    ...
    std::vector<SFObjs> radar_candidates = radar_obj_data_list_;

    auto apply_radar_fusion = [this, &radar_candidates](SFObjs &fusion_obj) {
        for (auto &radar_obj : radar_candidates) {
            if(radar_obj.confidence == 0.0){
                continue;
            }

            float iou = SF2Publisher::computeIoU(fusion_obj, radar_obj);
            if(iou >= 0.1){
                radar_obj.confidence = 0.0;
                fusion_obj.used_radar2_track_id_first = radar_obj.used_radar2_track_id_first;
                fusion_obj.used_radar2_track_id_second = radar_obj.used_radar2_track_id_second;

                // 카메라/라이다와 겹치는 레이더 트랙에 한해 속도 성분만 반영 (레이더 단독 노이즈는 fusion 결과에서 제외됨)
                fusion_obj.sf2_track_vel_x = radar_obj.sf2_track_vel_x;
                fusion_obj.sf2_track_vel_y = radar_obj.sf2_track_vel_y;
                break;
            }
        }
    };
    ...
    // 카메라/라이다와 겹치지 않는 레이더 단독 검출(노이즈 미필터링)은 fusion 결과에 포함하지 않음
    ...
    for (auto &obj : fusion_obj_data_list_) {
        ...
        msg_obj.used_radar2_track_id_first = obj.used_radar2_track_id_first;
        msg_obj.used_radar2_track_id_second = obj.used_radar2_track_id_second;
