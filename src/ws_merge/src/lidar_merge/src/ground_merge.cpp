...
public:
  ...
  : Node("lidar_merge_node")
  {
    ...
    sub_lidar1_ = this->create_subscription<PointCloud2>(
      "/TFnonground/lidar_11", 10, std::bind(&PointCloudMerger::callbackLidar1, this, _1));
    sub_lidar2_ = this->create_subscription<PointCloud2>(
      "/TFnonground/lidar_12", 10, std::bind(&PointCloudMerger::callbackLidar2, this, _1));
    sub_lidar3_ = this->create_subscription<PointCloud2>(
      "/TFnonground/lidar_13", 10, std::bind(&PointCloudMerger::callbackLidar3, this, _1));

    // Publisher
    pub_merged_ = this->create_publisher<PointCloud2>("/TFnonground/merged_points", 10);
    ...
    RCLCPP_INFO(this->get_logger(), "✅ Ground PointCloud Merge started.");
  }
