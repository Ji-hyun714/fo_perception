...
size_t TLensCalib::Load(void)
{
	...
	{
		//printf("file_name.c_str() : %s " , file_name.c_str());
		...
		ret = LoadV2(fp);
		Err_("sc 111 file_name.c_str() : %s  ", file_name.c_str());
		} else {
		ret = LoadV1(fp);
		}
...
int TLensCalib::UndistortFisheyePoints(vec2 *dst, vec2 const *src, int count)
{
	...
	printf("Distortion Coeffs D: %f %f %f %f\n", D[0], D[1], D[2], D[3]);

	printf("New Camera Matrix K_new:\n");
	printf("[ %f  %f  %f ]\n", K_new[0], K_new[1], K_new[2]);
	printf("[ %f  %f  %f ]\n", K_new[3], K_new[4], K_new[5]);
	printf("[ %f  %f  %f ]\n", K_new[6], K_new[7], K_new[8]);

	printf("Source Size: %d x %d\n", src_size.width, src_size.height);
