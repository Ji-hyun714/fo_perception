...
int32_t InferenceHelperTensorRt::Process(std::vector<OutputTensorInfo>& output_tensor_info_list)
{
    ...
    if (!context_->enqueueV2(buffer_list_gpu_.data(), stream, nullptr)) {
        PRINT_E("Failed to execute TensorRT enqueueV2\n");
        cudaStreamDestroy(stream);
        return kRetErr;
    }

    auto should_copy_output = [&output_tensor_info_list](int32_t binding_id) -> bool {
        for (const auto& output_tensor_info : output_tensor_info_list) {
            if (output_tensor_info.id == binding_id) {
                return output_tensor_info.is_copy_to_host;
            }
        }
        return false;
    };
    ...
    for (int i = 0; i < (int)buffer_list_cpu_.size(); i++) {
        if (!engine_->bindingIsInput(i) && should_copy_output(i)) {
