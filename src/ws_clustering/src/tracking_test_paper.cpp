...
private:
    ...
    vector<Detection> Clustering(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
    {
        ...
        rclcpp::Time scale_start = this->get_clock()->now();
        ...
        for (const auto& pt : cloud->points) {
            ...
            if (r > 10.0f) {
                ...
                z_scale = std::clamp(z_scale, 0.3f, 1.0f);
            }
        ...
        rclcpp::Time scale_end = this->get_clock()->now();
        double scale_ms = (scale_end - scale_start).seconds() * 1000.0;
        // std::cout << "scale: " << scale_ms << std::endl;

        scaling_total_ = scaling_total_ + scale_ms;
        frame_ = frame_ + 1;
    ...
    void Callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        // std::cout << "Scale_avg (ms): " << scaling_avg_ << std::endl;
