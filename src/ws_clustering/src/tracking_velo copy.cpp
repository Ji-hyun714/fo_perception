...
public:
    LidarKalmanNode() : Node("lidar_kalman_tracking"), next_id_(0) {
        ...
        pub_x_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/posx", 10);
        pub_y_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/posy", 10);
...
private:
    ...
    vector<Detection> Clustering(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
    {
        ...
        for (const auto& pt : cloud->points) {
            ...
            float z_scale = 1.0f;
            if (r > 10.0f) {
                z_scale = (1.0f/r) * 10.0f ;
                z_scale = std::clamp(z_scale, 0.5f, 1.0f);
            }

            // 수평 간격 보정
            float theta_h = std::atan2(pt.y, pt.x);
            float r_scale = 1.0f;
            if (r > 10.0f) {
                r_scale = (1.0f/r) * 10.0f;
                r_scale = std::clamp(r_scale, 0.7f, 1.0f);
            }

            // q.x = pt.x;
            // q.y = pt.y;
            q.x = r * r_scale * std::cos(theta_h);
            q.y = r * r_scale * std::sin(theta_h);
            q.z = pt.z * z_scale;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            while (!q.empty())
            {
                ...
                if (!visited[curr])
                {
                    ...
                    vector<float> new_dist;

                    tree->radiusSearch(curr, eps, new_neighbors, new_dist);

                    if ((int)new_neighbors.size() >= minPts)
                    {
                        for (int nn : new_neighbors)
                            q.push(nn);
                    }
                }
                ...
                if (!clustered[curr])
                {
                    const auto& pt = cloud->points[curr];
            ...
            // ================= 필터 조건 =================
            if (-10.0f < cx && cx < 10.0f && max_z < -0.3f) continue;  // 지면
            if (lz <= 0.25f && min_z > 0.7f) continue;
            if (min_z > 1.5f) continue;
            if (lz < 0.1f) continue;
            if (max_pt.x < 3.0f && min_pt.x > -2.0f && max_pt.y < 1.0f && min_pt.y > -1.0f) continue;
            // ===========================================
    ...
    void Prediction(Track& tr, double dt)
    {
        ...
        // Eigen::Matrix4f Q=Eigen::Matrix4f::Identity()*0.2f;
    ...
    void Callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        // const float MAX_ASSOC_DIST=1.0f;
        ...
        for(size_t i=0;i<N;i++)
            for(size_t j=0;j<M;j++){
                float d=(detections[i].x.head<2>()-
                        tracks_[j].x.head<2>()).norm();
                if(d<MAX_ASSOC_DIST)
                    cost[i][j]=d;
            }
        ...
        for(size_t i=0;i<N;i++){
            ...
            if(i<assignment.size() && j!=-1 && cost[i][j] < MAX_ASSOC_DIST){
                track_id = tracks_[assignment[i]].id;
        ...
        visualization_msgs::msg::MarkerArray x_markers;
        visualization_msgs::msg::MarkerArray y_markers;
        ...
        x_markers.markers.push_back(del);
        y_markers.markers.push_back(del);
        ...
        for(auto& tr:tracks_){
            ...
            float pos_rx = calib_cx - (tr.lx * 0.5);
            float pos_ry = calib_cy;

            std::ostringstream oss_x;
            oss_x << std::fixed << std::setprecision(2) << pos_rx;
            auto x_marker = createTextMarker(msg->header, "text_pos_x", tr.id, tr.cx, tr.cy, tr.cz, tr.lx, 0, oss_x.str());
            x_markers.markers.push_back(x_marker);

            float pos_y = pos_ry;
            std::ostringstream oss_y;
            oss_y << std::fixed << std::setprecision(2) << pos_y;
            auto y_marker = createTextMarker(msg->header, "text_pos_y", tr.id, tr.cx, tr.cy, tr.cz, tr.lx, 0, oss_y.str());
            y_markers.markers.push_back(y_marker);
            ...
            point_marker.pose.position.z = tr.cz - (tr.lz * 0.5);
        ...
        pub_x_->publish(x_markers);
        pub_y_->publish(y_markers);
    ...
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray >::SharedPtr pub_x_;
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray >::SharedPtr pub_y_;
