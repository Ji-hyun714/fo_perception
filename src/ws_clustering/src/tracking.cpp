...
public:
    LidarKalmanNode() : Node("lidar_kalman_tracking"), next_id_(0) {
        ...
        pub_bbox_ = create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/bbox2", 10);
        pub_bbox_vehicle_ = create_publisher<visualization_msgs::msg::MarkerArray>("/lidar/bbox2", 10);
        pub_id_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/id2", 10);
        pub_z_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/maxz2", 10);
        pub_lz_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/lz2", 10);
        pub_point_ = this->create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_markers/point2", 10);
        pub_sf_ = this->create_publisher<fo_msgs::msg::LidarobjListsForSF2>("/lidar/sf_objs2", 10);
        pub_cluster_ = create_publisher<sensor_msgs::msg::PointCloud2>("/lidar/cluster_points2", 10);
...
private:
        ...
        const sensor_msgs::msg::PointCloud2::SharedPtr& msg)
    {
        ...
        // scaling_total_ = scaling_avg_ + preprocess_ms;
        // frame_ = frame_ + 1;
        // scaling_avg_ = scaling_total_ / frame_;
    ...
    vector<Detection> Clustering(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
    {
        ...
        //         z_scale = std::clamp(z_scale, 0.4f, 1.0f);
        //     }
        ...
        // process_total_ = process_total_ + clustering_ms;
        // process_avg_ = process_total_ / frame_;

        // std::cout << "Process_avg (ms): " << process_avg_ << std::endl;
        ...
        const std::string& text)
    {
        ...
        marker.pose.position.y = y - offset;
    ...
    void Callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        for (auto& tr : tracks_) {
            ...
            mk.color.g = (tr.missed == 0) ? 0.0f : 1.0f;
            mk.color.b = (tr.missed == 0) ? 1.0f : 0.0f;
            mk.color.a = 0.4f;
            ...
            auto id_marker = createTextMarker(msg->header, "text_id", tr.id, tr.cx, tr.cy, tr.cz, tr.ly, tr.ly*0.5, std::to_string(tr.id));
        ...
        //               << "lz: " << tr.lz << " / "
        //               << "tracked: " << tr.tracked << " / "
        //               << "min_pt.z: " << tr.cz - (tr.lz*0.5) << std::endl;
        //     //         //   << "vel_x: " << tr.x(2) << " / "
        //     //         //   << "vel_y: " << tr.x(3) << " / "
        //     //         //   << "vel: " << tr.vel << " / "
        ...
        rclcpp::Time pub = this->get_clock()->now();

        double process_ms = (pub - sub).seconds() * 1000.0;
        double process_total = process_total + process_ms;
        int frame_ = frame_ + 1;
        double process_avg_ = process_total / frame_;

        std::cout << "Existing_Process(ms): " << process_avg_ << std::endl;
    ...
    double scaling_avg_{0.0};
    double scaling_total_{0.0};
    double process_avg_{0.0};
    double process_total_{0.0};
    int frame_{0};
