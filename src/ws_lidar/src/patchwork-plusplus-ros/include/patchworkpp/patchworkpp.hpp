...
public:
    ...
    PatchWorkpp(const rclcpp::NodeOptions &options)
        ...
        sub_cloud = Node::create_subscription<sensor_msgs::msg::PointCloud2>(cloud_topic, 1, std::bind(&PatchWorkpp<PointT>::callbackCloud, this, std::placeholders::_1));    
        ...
        // RCLCPP_INFO(this->get_logger(), "set up: keep_last 1");
        // RCLCPP_INFO(this->get_logger(), "set up: queue 1");
...
private:
    ...
    // rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_revert_pc, pub_reject_pc, pub_normal, pub_noise, pub_vertical, pub_cloud, pub_non_ground;
...
void PatchWorkpp<PointT>::callbackCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg)
{
    ...
    crop.setInputCloud(pc_curr.makeShared());
    ...
    // voxelize
    typename pcl::PointCloud<PointT>::Ptr ds(new pcl::PointCloud<PointT>);
    ...
    estimate_ground(pc_curr, pc_ground, pc_non_ground);
    // if(display_time_){
