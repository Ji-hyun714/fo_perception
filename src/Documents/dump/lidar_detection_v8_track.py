...
class EnhancedObjectDetectionNode(Node):
    def __init__(self):
        ...
        # Start a separate thread to handle publishing of bounding boxes
        self.publish_thread = threading.Thread(target=self.publish_boxes_periodically)
    ...
    def create_marker(self, bbox, marker_id):
        ...
        marker.lifetime = Duration(sec=1)  # 1초 동안 유지
        return marker
