...
class LiDARTransformer(Node):
    def __init__(self):
        ...
        self.lidar_sub = self.create_subscription(
            PointCloud2,
            # '/lidar_11_201/points',  # LiDAR 토픽 이름
            '/nonground/lidar_11',  # LiDAR 토픽 이름
            self.lidar_callback,
            1
        )

        # 변환된 LiDAR 데이터를 새로운 토픽으로 발행
        # self.transformed_pub = self.create_publisher(PointCloud2, '/lidar_11_201/TFpoints', 1)
        self.transformed_pub = self.create_publisher(PointCloud2, '/TFnonground/lidar_11', 1)

        # 변환 값 (translation과 rotation)
        self.translation = np.array([2.2, 1, 0])  # X, Y, Z 평행이동
        self.rotation_matrix = tf_transformations.quaternion_matrix([0, 0, 0.552199, 0.833712])[:3, :3]  # 3x3 회전 행렬 생성
        
        self.get_logger().info("Translate LiDAR_11 points")
