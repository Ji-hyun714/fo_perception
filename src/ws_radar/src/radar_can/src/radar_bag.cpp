...
RadarBagNode::RadarBagNode()
: Node("radarBag_thread_node")
{}
...
bool RadarBagNode::init() {
    this->declare_parameter<std::string>("can_channel", "can0");
    this->declare_parameter<int>("delay_threshold_ms", DELAY_THRESHOLD_MS);
    this->declare_parameter<int>("lost_threshold_ms", LOST_THRESHOLD_MS);

    this->get_parameter("can_channel", can_channel_);
    this->get_parameter("delay_threshold_ms", delay_ms);
    this->get_parameter("lost_threshold_ms", lost_ms);

    delay_threshold_ = delay_ms / 1000.0;
    lost_threshold_  = lost_ms  / 1000.0;
    ...
    pub_can_frame_ = this->create_publisher<fo_msgs::msg::CanFrame>("/radar_frame", QOS_QUEUE);
    ...
    // radar_general timer
    timer_general = this->create_wall_timer(TIMER_PERIOD, std::bind(&RadarBagNode::general_callback, this));

    // CAN 수신 전용 스레드 시작
    running_ = true;
    recv_thread_ = std::thread(&RadarBagNode::recv_loop, this);

    return true;
}


bool RadarBagNode::open_can_socket() {
...
void RadarBagNode::general_callback() {
    auto now = this->now();
...
void RadarBagNode::radar_bag(const struct can_frame &frame) {

    last_msg_time_ = this->now();

    fo_msgs::msg::CanFrame msg;
    msg.header.stamp = last_msg_time_;
    msg.header.frame_id = "can0"; 
    msg.can_id = frame.can_id;
    msg.len = frame.len;
    msg.pad = frame.__pad;
    msg.res0 = frame.__res0;
    msg.len8_dlc = frame.len8_dlc;

    for (int i = 0; i < frame.len; i++) {
        msg.data[i] = frame.data[i];
    }
    pub_can_frame_->publish(msg);
...
int main(int argc, char* argv[]) {
    ...
    auto node = std::make_shared<RadarBagNode>();
    if (!node->init()) {
        RCLCPP_ERROR(node->get_logger(), "Failed to initialize RadarBagNode");
        return 1;
    }
